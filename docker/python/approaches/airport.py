...
class airport(Approach):
    ...
    def process_round(self, state: GameState):
        ...
        for city in cities:
            ...
            connected_cities = list(map(lambda connection_name: self.get_city_for_name(cities, connection_name),
            ...
            for other_city in connected_cities:
                ...
                for pathogen_in_city in set(pathogens_in_city).difference(pathogens_in_other_city):
                    priority_for_connection += self.calculate_priority_for_connection(city, other_city,
                                                                                      pathogen_in_city)

                # the closing of a connection, unlike the closing of an airport, is not bidirectional
                rounds = 1
                while available_points >= self.cost_for_connection_close(rounds) and rounds <= 6:
                    effectiveness = priority_for_connection * rounds / self.cost_for_connection_close(rounds)
                    action = close_airway(city.index, other_city.index, rounds)
                    possible_actions.append(Action(effectiveness, action))
                    rounds += 1

            if airport_open:
                for other_city in map(lambda city_name: self.get_city_for_name(cities, city_name), city.connections):
                    priority_for_connection = 0
                    pathogens_in_other_city = other_city.pathogens

                    for pathogen_in_other_city in set(pathogens_in_other_city).difference(pathogens_in_city):
                        priority_for_connection += self.calculate_priority_for_connection(other_city, city,
                ...
                while available_points >= self.cost_for_airport_close(rounds):
                    effectiveness = priority_for_city * rounds / self.cost_for_airport_close(rounds)
    ...
    @classmethod
    def get_open_connections_for_city(cls, city):
    ...
    @classmethod
    def get_city_for_name(cls, cities, city_name) -> City:
        return list(filter(lambda city: city.name == city_name, cities))[0]

    @classmethod
    def calculate_priority_for_connection(cls, infected_city, uninfected_city, pathogen):
    ...
    @classmethod
    def cost_for_connection_close(cls, rounds):
        return 3 + (rounds * 3)

    @classmethod
    def cost_for_airport_close(cls, rounds):
        return 15 + (rounds * 5)
